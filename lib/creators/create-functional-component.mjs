...
function createCode({ root, name, style }) {
	const codeTemplate = nunjucks.render(`./lib/schemas/functional-component/code.html`, { name, styleExtension: style.extension });
...
function createStyle({ root, name, style }) {
	const styleTemplate = nunjucks.render(`./lib/schemas/functional-component/style.html`, { name: name.toLowerCase() });
...
function createSpec({ root, name }) {
	const specTemplate = nunjucks.render(`./lib/schemas/functional-component/spec.html`, { name });
...
function createStories({ root, name }) {
	const storiesTemplate = nunjucks.render(`./lib/schemas/functional-component/stories.html`, { name });
