...
export function createComponentConnectedToStore(component, options) {
	const configuration = {
...
function createCode({ root, name, style }) {
	const codeTemplate = nunjucks.render(`./lib/schemas/component-connected-to-store/code.html`, { name, styleExtension: style.extension });
...
function createStyle({ root, name, style }) {
	const styleTemplate = nunjucks.render(`./lib/schemas/component-connected-to-store/style.html`, { name: name.toLowerCase() });
...
function createSpec({ root, name }) {
	const specTemplate = nunjucks.render(`./lib/schemas/component-connected-to-store/spec.html`, { name });
...
function createStories({ root, name }) {
	const storiesTemplate = nunjucks.render(`./lib/schemas/component-connected-to-store/stories.html`, { name });
